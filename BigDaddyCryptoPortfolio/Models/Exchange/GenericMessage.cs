...
namespace BigDaddyCryptoPortfolio.Models.Exchange
{
	public struct GenericMessage
	{
        ...
        public static GenericMessage CreateAesEncryptedMessage(GenericMessageType messageType, byte[] data, byte[] key, byte[] iv)
        {
            return new GenericMessage()
            {
                MessageId = Guid.NewGuid(),
                MessageType = messageType,
                Payload = Toolkit.AesEncrypt(data, key, iv)
            };
        }
    }
}
